...
void bellmanFord(grafo **g, int partida);
    ...
    for(int i=0; i<80 && altered;i++){
        ...
        for(int k=0;k<81;k++){
            if((*g)->shortTable[k][1]!=INT_MAX){
                cost = (*g)->shortTable[k][1]+1;
                int treshold = k!=40 && k!=0 && k!=80 ? 3 : 2;
                for(int j=0;j<treshold;j++){
                    if((*g)->shortTable[(*g)->adjacencias[k][j]][1] > cost){
                        (*g)->shortTable[(*g)->adjacencias[k][j]][0] = k;
                        (*g)->shortTable[(*g)->adjacencias[k][j]][1] = cost;
                        altered = True;
                    }
                }
            }
